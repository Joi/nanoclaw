...
function testMigration(name: string, table = name.replace(/[^a-zA-Z0-9_]/g, '_')): Migration {
  return {
    version: 999,
    name,
    up(db) {
...
function testModuleMigration(name: ModuleMigrationName, table?: string): ModuleMigration {
  return testMigration(name, table) as ModuleMigration;
}
...
describe('module migration registry', () => {
  it('keeps built-ins first and module migrations in registration order', async () => {
    ...
    const first = testModuleMigration('module:test-first:create-state');
    const second = testModuleMigration('module:test-second:create-state');
